...
func (t *SkillTool) Execute(ctx context.Context, args map[string]any) *tools.ToolResult {
	...
	// Build the full command: python3 skills/<name>/<entrypoint> <parts...>
	fullArgs := append([]string{t.entrypoint}, parts...)
...
func (a *WorkerApp) RegisterSkills(inst *agent.AgentInstance, chatID string) {
	...
	for _, entry := range entries {
		...
		// Find the engine script (strictly main.py)
		entrypoint := ""
		if _, err := os.Stat(filepath.Join(skillPath, "main.py")); err == nil {
			entrypoint = filepath.Join("skills", skillName, "main.py")
		}
